...
const DispatchPage = () => {
  ...
  /**
   * Loads the branch list on mount. The initial dispatch-report fetch is
   * triggered once by the effect above, after branches resolve and the
   * default date-range filter is set — fetching dispatches here too would
   * duplicate that request with a still-unfiltered branch list.
   */
  useEffect(() => {
    const loadBranches = async () => {
      ...
      try {
        ...
        // If there are no branches, the branchOptions effect below never
        // fires fetchDataWithFilter (which owns isLoading afterwards) — so
        // release the spinner here too rather than leaving it stuck.
        setIsLoading(false);
      }
    };
    loadBranches();
