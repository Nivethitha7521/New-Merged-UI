...
export const fetchVendorTypeItems = createAsyncThunk<VendorTypeItem[], { signal?: AbortSignal }>(
  ...
  async ({ signal }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get('http://192.168.1.131:8000/purchaseapi/vendortypes/', {
...
export const addVendorTypeItem = createAsyncThunk<VendorTypeItem, { data: VendorTypeItem; signal?: AbortSignal }>(
  ...
  async ({ data, signal }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post('http://192.168.1.131:8000/purchaseapi/vendortypes', data, {
        cancelToken: source.token,
        headers: getAuthHeaders() // ✅ ADD HEADERS
      });
      return response.data;
...
>(
  ...
  async ({ vendortypeId, vendortype, signal }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.patch(`http://192.168.1.131:8000/purchaseapi/vendortypes/${vendortypeId}`, vendortype, {
        cancelToken: source.token,
        headers: getAuthHeaders() // ✅ ADD HEADERS
      });
      return response.data;
    ...
    try {
      ...
      signal?.addEventListener('abort', () => {
       ...
       const response = await axios.patch(
        `http://192.168.1.131:8000/purchaseapi/vendortypes/${vendortypeId}/deactivate`,
        {},
        { cancelToken: source.token,
  ...
  async ({ vendortypeId, signal }, { rejectWithValue }) => {
    try {
      ...
      signal?.addEventListener('abort', () => {
       ...
       const response = await axios.patch(
        `http://192.168.1.131:8000/purchaseapi/vendortypes/${vendortypeId}/activate`,
        {},
        { cancelToken: source.token,
          headers: getAuthHeaders() }
      );
