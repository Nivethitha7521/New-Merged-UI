...
export const fetchPermissions = createAsyncThunk(
  ...
  async (): Promise<PermissionRecord[]> => {
    const response = await fetch('http://192.168.1.131:8000/purchaseapi/permissions');
...
export const updatePermissions = createAsyncThunk(
  ...
  async ({ roleName, permissions }: UpdatePermissionsPayload): Promise<PermissionRecord> => {
    const response = await fetch(`http://192.168.1.131:8000/purchaseapi/permissions/${roleName}`, {
      method: 'PATCH',
