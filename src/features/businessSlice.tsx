...
export const fetchBusinesses = createAsyncThunk('businesses/fetchBusinesses', async () => {
  const response = await axios.get('http://192.168.1.102:8000/purchaseapi/pobusiness/'); // Adjust API endpoint as needed
...
export const addBusiness = createAsyncThunk<Business, Business>('businesses/addBusiness', async (businessData) => {
  const response = await axios.post('http://192.168.1.102:8000/purchaseapi/pobusiness/', businessData); // Adjust API endpoint as needed
  return response.data;
});
...
export const updateBusiness = createAsyncThunk<Business, Business>('businesses/updateBusiness', async (businessData) => {
  const response = await axios.patch(`http://192.168.1.102:8000/purchaseapi/pobusiness/${businessData.businessId}`, businessData); // Adjust API endpoint as needed
  return response.data;
});
  ...
  async ({ businessId, file }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        `http://192.168.1.102:8000/purchaseapi/pobusiness/upload?custom_id=${businessId}`, // Use the business ID in the URL
        formData,
        {
          headers: {
...
export const fetchPhoto = createAsyncThunk(
  ...
  async (businessId: string, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://192.168.1.102:8000/purchaseapi/pobusiness/view/${businessId}`, { responseType: 'blob' });
...
export const fetchShipping = createAsyncThunk('shipping/fetchShipping', async () => {
  const response = await axios.get('http://192.168.1.102:8000/purchaseapi/poshippingaddress/'); // Adjust API endpoint as needed
  return response.data;
});
