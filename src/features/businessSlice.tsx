...
export const fetchBusinesses = createAsyncThunk('businesses/fetchBusinesses', async () => {
  const response = await axios.get('http://127.0.0.1:8000/yenerpapi/pobusiness/'); // Adjust API endpoint as needed
...
export const addBusiness = createAsyncThunk<Business, Business>('businesses/addBusiness', async (businessData) => {
  const response = await axios.post('http://127.0.0.1:8000/yenerpapi/pobusiness/', businessData); // Adjust API endpoint as needed
  return response.data;
});
...
export const updateBusiness = createAsyncThunk<Business, Business>('businesses/updateBusiness', async (businessData) => {
  const response = await axios.patch(`http://127.0.0.1:8000/yenerpapi/pobusiness/${businessData.businessId}`, businessData); // Adjust API endpoint as needed
  return response.data;
});
  ...
  async ({ businessId, file }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        `http://127.0.0.1:8000/yenerpapi/pobusiness/upload?custom_id=${businessId}`, // Use the business ID in the URL
        formData,
        {
          headers: {
...
export const fetchPhoto = createAsyncThunk(
  ...
  async (businessId: string, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/yenerpapi/pobusiness/view/${businessId}`, { responseType: 'blob' });
...
export const fetchShipping = createAsyncThunk('shipping/fetchShipping', async () => {
  const response = await axios.get('http://127.0.0.1:8000/yenerpapi/poshippingaddress/'); // Adjust API endpoint as needed
  return response.data;
});
 
// Async thunk to add a new Business item
export const addShipping = createAsyncThunk<ShippingAddress, ShippingAddress>('shipping/addShipping', async (shippingaddress) => {
  const response = await axios.post('http://127.0.0.1:8000/yenerpapi/poshippingaddress/', shippingaddress); // Adjust API endpoint as needed
  return response.data;
});
...
export const updateShipping = createAsyncThunk<ShippingAddress, ShippingAddress>('shipping/updateBusiness', async (shippingaddress) => {
  const response = await axios.patch(`http://127.0.0.1:8000/yenerpapi/poshippingaddress/${shippingaddress.shippingId}`, shippingaddress); // Adjust API endpoint as needed
  return response.data;
});
