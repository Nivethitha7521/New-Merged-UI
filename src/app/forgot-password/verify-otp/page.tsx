...
const verifyOtp = async () => {
  ...
  `http://192.168.1.100:8000/purchaseapi/users/verify-otp?username=${encodeURIComponent(username!)}&otp=${otp}`,
  { method: "POST" }
);
  ...
  const resendOtp = async () => {
    try {
     const res = await fetch(
  `http://192.168.1.100:8000/purchaseapi/users/resend-otp?username=${encodeURIComponent(username!)}`,
  { method: "POST" }
);
