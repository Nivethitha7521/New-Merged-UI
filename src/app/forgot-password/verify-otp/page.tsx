...
const verifyOtp = async () => {
  ...
  `http:// 192.168.1.111:8000/purchasetestapi/users/verify-otp?username=${encodeURIComponent(username!)}&otp=${otp}`,
  { method: "POST" }
);
  ...
  const resendOtp = async () => {
    try {
     const res = await fetch(
  `http:// 192.168.1.111:8000/purchasetestapi/users/resend-otp?username=${encodeURIComponent(username!)}`,
  { method: "POST" }
);
