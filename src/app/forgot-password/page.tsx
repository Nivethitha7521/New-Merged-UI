...
const res = await fetch(
  `http://192.168.1.102:8000/purchaseapi/users/forgot-password?${query.toString()}`,
  { method: "POST" }
);
...
sessionStorage.setItem("fp_username", username);
 ...
 setIsAutoFetching(true);
  try {
    const res = await fetch(
     `http://192.168.1.102:8000/purchaseapi/users/email/${encodeURIComponent(mail)}/username`
...
const fetchEmailByUsername = async (uname: string) => {
 ...
 ;
  try {
    const res = await fetch(
    `http://192.168.1.102:8000/purchaseapi/users/username/${encodeURIComponent(uname)}/email`
