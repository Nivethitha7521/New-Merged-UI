...
export default function NewPassword() {
  ...
  useEffect(() => {
    ...
    const storedOtp = sessionStorage.getItem("fp_otp_verified");

    if (!storedIdentifier || !storedOtp) {
  ...
  const isPasswordInvalid =
    passwordTouched && password.length > 0 && !PASSWORD_REGEX.test(password);
  ...
  const resetPassword = async () => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/yenerpapi/auth/reset-password`,
        {
          method: "POST",
      ...
      if (!res.ok) {
        toast.error(data.detail || "Unable to reset password");
        return;
      }
      ...
      sessionStorage.removeItem("fp_otp_verified_value");
      router.push("/");
    } catch {
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
          ...
          <div className="text-center mb-4">
            ...
            <p className="text-gray-500 mt-1">
              Create a strong new password. This will also unlock your account
