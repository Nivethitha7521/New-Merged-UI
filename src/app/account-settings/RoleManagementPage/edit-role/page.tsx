...
setFormPermissions(applyDefaultHide(JSON.parse(JSON.stringify(HARD_MODULES))));
      ...
      try {
        // Fetch role
        const roleRes = await authFetch(`https://yenerp.com/purchasetestapi/roles?name=${encodeURIComponent(roleName)}`);
        if (roleRes.ok) {
        ...
        // Fetch permissions
        // Fetch permissions
        const permRes = await authFetch(`https://yenerp.com/purchasetestapi/permissions?role_name=${encodeURIComponent(roleName)}`);
        if (permRes.ok) {
...
const backendPerms = transformPermissionsForBackend(sanitizedPermissions);
      ...
      if (formRoleName !== roleName) {
        try {
          ...
          if (roleToUpdate && roleToUpdate._id) {
            await authFetch(`https://yenerp.com/purchasetestapi/roles/${roleToUpdate._id}`, {
              method: "PATCH",
      ...
      let response = await authFetch(`https://yenerp.com/purchasetestapi/permissions/${encodeURIComponent(roleName)}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      if (response.status === 404) {
        ...
        response = await authFetch("https://yenerp.com/purchasetestapi/permissions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (formRoleName !== roleName) {
        try {
          ...
          const checkResponse = await authFetch(`https://yenerp.com/purchasetestapi/permissions?role_name=${encodeURIComponent(formRoleName)}`);
          ...
          if (!checkData || (Array.isArray(checkData) && checkData.length === 0)) {
            ...
            await authFetch("https://yenerp.com/purchasetestapi/permissions", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            await authFetch(`https://yenerp.com/purchasetestapi/permissions/${encodeURIComponent(roleName)}`, {
              method: "DELETE"
            });
          }
        } catch (checkErr) {
