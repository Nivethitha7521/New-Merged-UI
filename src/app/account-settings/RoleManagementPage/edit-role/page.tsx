...
setFormPermissions(applyDefaultHide(JSON.parse(JSON.stringify(HARD_MODULES))));
      ...
      try {
        // Fetch role
        const roleRes = await fetch(`http://192.168.1.102:8000/purchaseapi/roles?name=${encodeURIComponent(roleName)}`);
        if (roleRes.ok) {
        ...
        // Fetch permissions
        const permRes = await fetch(`http://192.168.1.102:8000/purchaseapi/permissions?role_name=${encodeURIComponent(roleName)}`);
        if (permRes.ok) {
...
const backendPerms = transformPermissionsForBackend(sanitizedPermissions);
      ...
      if (formRoleName !== roleName) {
        try {
          ...
          if (roleToUpdate && roleToUpdate._id) {
            await fetch(`http://192.168.1.102:8000/purchaseapi/roles/${roleToUpdate._id}`, {
              method: "PATCH",
      ...
      let response = await fetch(`http://192.168.1.102:8000/purchaseapi/permissions/${encodeURIComponent(roleName)}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      if (response.status === 404) {
        ...
        response = await fetch("http://192.168.1.102:8000/purchaseapi/permissions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (formRoleName !== roleName) {
        try {
          ...
          const checkResponse = await fetch(`http://192.168.1.102:8000/purchaseapi/permissions?role_name=${encodeURIComponent(formRoleName)}`);
          ...
          if (!checkData || (Array.isArray(checkData) && checkData.length === 0)) {
            ...
            await fetch("http://192.168.1.102:8000/purchaseapi/permissions", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            await fetch(`http://192.168.1.102:8000/purchaseapi/permissions/${encodeURIComponent(roleName)}`, {
              method: "DELETE"
            });
          }
        } catch (checkErr) {
