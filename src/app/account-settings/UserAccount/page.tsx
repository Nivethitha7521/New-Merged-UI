...
const [passwordTouched, setPasswordTouched] = useState(false);
  ...
  const fetchRolesFromBackend = async () => {
    try {
      ...
      const response = await fetch('http://192.168.1.109:8000/purchasetestapi/roles');
...
const fetchRoleAppsFromPermissions = async () => {
  try {
    ...
    const res = await fetch("http://192.168.1.109:8000/purchasetestapi/permissions");
...
if (
  ...
  if (!PASSWORD_REGEX.test(formUser.password)) {
    ...
    if (editingUserId) {
      ...
      const response = await fetch(`http://192.168.1.109:8000/purchasetestapi/users/${editingUserId}`, {
        method: 'PUT',
        headers: {
...
setSnackbar({
  ...
  severity: "success",
      ...
      const response = await fetch('http://192.168.1.109:8000/purchasetestapi/users', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
setSnackbar({
 ...
 const handleDeactivateUser = async (userId: string) => {
  try {
    // 🔥 1. Update backend
    await fetch(`http://192.168.1.109:8000/purchasetestapi/users/${userId}/deactivate`, {
      method: "PATCH",
    });
 ...
 const handleRestoreUser = async (userId: string) => {
  try {
    await fetch(`http://192.168.1.109:8000/purchasetestapi/users/${userId}/activate`, {
      method: "PATCH",
    });
