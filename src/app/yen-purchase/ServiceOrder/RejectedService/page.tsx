...
// ACTION PERMISSIONS
const canRead = hasPermission("yenerp", "serviceorders_rejected", "read");
const canEdit = hasPermission("yenerp", "serviceorders_rejected", "edit");
...
if (!isRejectedModuleVisible) {
  return (
    <Box p={3}>
      <Alert severity="error">
        You do not have access to Rejected Service module.
      </Alert>
    </Box>
  );
}
...
if (!canRead) {
  return (
    <Box p={3}>
      <Alert severity="error">
        You do not have permission to view Rejected Service Orders.
      </Alert>
    </Box>
  );
}
    ...
    <Box>
      ...
      <Box sx={{ px: 2, py: 1 }}>
        ...
        <Grid container spacing={2} sx={{ mb: 1 }}>
          ...
          {canReadPending && (
            <Link href={"/yen-purchase/ServiceOrder"}>
            ...
            {canReadApproved && (
            <Link href={"/yen-purchase/ServiceOrder/ApprovedService"}>
              <Button variant="contained" color="primary" sx={{ ml: 1 }}>
                Approved
              </Button>
            </Link>
            )}
            <Link href={"/yen-purchase/ServiceOrder/RejectedService"}>
              <Button
                variant="contained"
                sx={{
                  ml: 1,
...
>
  ...
  disabled={!canDelete}
  sx={{
    color: canDelete ? "error.main" : "#6e6e6e !important",
    opacity: 1,
