'use client';
import React, { useEffect, useRef, useState } from 'react';
import {
  Dialog,
  DialogTitle,
  DialogContent,
  DialogActions,
  Button,
  TextField,
  CircularProgress,
  Dialog as ConfirmDialog,
  DialogContentText,
} from '@mui/material';
import { Formik, Form, FormikHelpers } from 'formik';
import * as Yup from 'yup';
import { Brand } from '../Models/BrandModel';

const validationSchema = Yup.object({
  brandName: Yup.string()
    .required('Brand name is required')
    .max(24, 'Maximum 24 characters required')
    .matches(/^[a-zA-Z0-9 ]*$/, 'Brand name must only contain letters, numbers, and spaces')
    .test('no-leading-trailing-spaces', 'Brand name cannot start or end with spaces', (value) => {
      if (!value) return false;
      return value === value.trim();
    })
    .test('no-only-spaces', 'Brand name cannot be only spaces', (value) => {
      if (!value) return false;
      return value.trim().length > 0;
    }),
});

interface BrandFormProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (values: Brand, formikHelpers: FormikHelpers<Brand>) => Promise<void>;
  initialValues: Brand;
  editIndex: number | null;
  loading: boolean;
}

const BrandForm: React.FC<BrandFormProps> = ({
  open,
  onClose,
  onSubmit,
  initialValues,
  editIndex,
  loading,
}) => {
  const [showConfirmDialog, setShowConfirmDialog] = useState(false);
  const [unsavedChanges, setUnsavedChanges] = useState(false);
  const [confirmSubmitDialog, setConfirmSubmitDialog] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [pendingSubmit, setPendingSubmit] = useState<(() => Promise<void>) | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  const handleClose = (formikDirty: boolean) => {
    if (formikDirty || unsavedChanges) {
      setShowConfirmDialog(true);
    } else {
      onClose();
    }
  };

  useEffect(() => {
    if (open) {
      const timer = setTimeout(() => {
        if (inputRef.current) {
          inputRef.current.focus();
          const length = inputRef.current.value.length;
          inputRef.current.setSelectionRange(length, length);
        }
      }, 100);
      setIsSubmitting(false);
      setUnsavedChanges(false);
      setPendingSubmit(null);
      return () => clearTimeout(timer);
    }
  }, [open]);

  const handleConfirmClose = () => {
    setShowConfirmDialog(false);
    setUnsavedChanges(false);
    setIsSubmitting(false);
    onClose();
  };

  const handleSubmitClick = (submitForm: () => Promise<void>) => {
    setPendingSubmit(() => submitForm);
    setConfirmSubmitDialog(true);
  };

  const handleConfirmSubmit = async () => {
    if (pendingSubmit) {
      setIsSubmitting(true);
      setConfirmSubmitDialog(false);
      try {
        await pendingSubmit();
      } catch (error) {
        // Error handled in onSubmit
      } finally {
        setPendingSubmit(null);
        setIsSubmitting(false);
      }
    }
  };

  return (
    <>
      <Dialog open={open} onClose={() => handleClose(unsavedChanges)}>
        <DialogTitle>
          {editIndex !== null ? 'Edit Brand' : 'Add New Brand'}
        </DialogTitle>
        <Formik
          initialValues={initialValues}
          validationSchema={validationSchema}
          onSubmit={async (values, formikHelpers) => {
            const finalValues = {
              ...values,
              brandName: values.brandName.trim(),
            };
            try {
              await onSubmit(finalValues, formikHelpers);
              setUnsavedChanges(false);
              onClose();
            } catch (error: any) {
              formikHelpers.setFieldError('brandName', error.message || 'Failed to save brand');
            } finally {
              setIsSubmitting(false);
            }
          }}
        >
          {({
            values,
            errors,
            touched,
            handleChange,
            handleBlur,
            dirty,
            setFieldValue,
            submitForm,
          }) => (
            <Form>
              <DialogContent>
                <TextField
                  inputRef={inputRef}
                  autoComplete="off"
                  margin="dense"
                  id="brandName"
                  name="brandName"
                  label="Brand Name"
                  type="text"
                  fullWidth
                  variant="outlined"
                  value={values.brandName}
                  onChange={(e) => {
                    let newValue = e.target.value.replace(/\s+/g, ' ').trimStart();
                    setFieldValue('brandName', newValue);
                    setUnsavedChanges(true);
                  }}
                  onBlur={(e) => {
                    const trimmedValue = e.target.value.trim();
                    setFieldValue('brandName', trimmedValue);
                    handleBlur(e);
                  }}
                  error={touched.brandName && !!errors.brandName}
                  helperText={touched.brandName && errors.brandName}
                  InputLabelProps={{
                    sx: {
                      '&::after': {
                        content: '"*"',
                        color: 'red',
                        marginLeft: '4px',
                      },
                    },
                  }}
                />
              </DialogContent>
              <DialogActions>
                <Button onClick={() => handleClose(dirty)} color="primary" disabled={isSubmitting}>
                  Cancel
                </Button>
                <Button
                  type="button"
                  color="primary"
                  disabled={isSubmitting || loading || !values.brandName.trim()}
                  startIcon={isSubmitting || loading ? <CircularProgress size={24} /> : null}
                  onClick={() => handleSubmitClick(submitForm)}
                >
                  {isSubmitting || loading ? 'Processing...' : editIndex !== null ? 'Save Changes' : 'Add'}
                </Button>
              </DialogActions>
            </Form>
          )}
        </Formik>
      </Dialog>

      {/* Unsaved Changes Dialog */}
      <ConfirmDialog open={showConfirmDialog} onClose={() => setShowConfirmDialog(false)}>
        <DialogTitle>Unsaved Changes</DialogTitle>
        <DialogContent>
          <DialogContentText>
            You have unsaved changes. Are you sure you want to close the dialog without saving?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setShowConfirmDialog(false)} color="primary" disabled={isSubmitting}>
            Cancel
          </Button>
          <Button onClick={handleConfirmClose} color="primary" disabled={isSubmitting}>
            Confirm
          </Button>
        </DialogActions>
      </ConfirmDialog>

      {/* Confirm Submit Dialog */}
      <ConfirmDialog open={confirmSubmitDialog} onClose={() => setConfirmSubmitDialog(false)}>
        <DialogTitle>{editIndex !== null ? 'Confirm Edit' : 'Confirm Add'}</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure you want to {editIndex !== null ? 'save the changes' : 'add this brand'}?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setConfirmSubmitDialog(false)} color="primary" disabled={isSubmitting}>
            Cancel
          </Button>
          <Button onClick={handleConfirmSubmit} color="primary" disabled={isSubmitting}>
            Confirm
          </Button>
        </DialogActions>
      </ConfirmDialog>
    </>
  );
};

export default BrandForm;