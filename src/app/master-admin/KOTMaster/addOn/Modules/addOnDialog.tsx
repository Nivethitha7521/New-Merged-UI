...
import { BranchwiseItems } from '../Models/addonModels';
...
const AddOnDialog: React.FC<AddOnDialogProps> = ({
  ...
  // Focus AddOn name field on open
  useEffect(() => {
    if (open && inputRef.current) {
      const timer = setTimeout(() => {
        inputRef.current?.focus();
        if (addOnData.id) inputRef.current?.select();
      }, 100);
      return () => clearTimeout(timer);
    }
  }, [open, addOnData.id]);

  // Sync selected items from props (important on edit/open)
  useEffect(() => {
    if (open) {
      setSelectedItems(addOnData.addOnItems || []);
      setHighlightedSelected([]);
    }
  }, [open, addOnData.addOnItems]);
    ...
    >
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
          ...
          <Box sx={{ flex: '0 0 180px' }}>
            <TextField
              ...
              required
              inputRef={inputRef}
              error={!!validationErrors.addOn || hasLetterError(addOnData.addOn)}
              helperText={ getHelperText(addOnData.addOn, validationErrors.addOn)}
              ...
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    placeholder={addOnData.addOnItems?.length === 0 ? "Search and select variances" : ""}
                    margin="none"
                    fullWidth
                    error={!!validationErrors.addOnItems}
                    helperText={validationErrors.addOnItems}
                    className="custom-textfield"
                    InputLabelProps={{ className: "custom-label" }}
                    onKeyDown={(e) => {
                      if (e.key === 'Backspace') {
      ...
      <DialogActions className='dialog-actions'>
        ...
        <button
          ...
          disabled={isSubmitting || !addOnData.addOn.trim() || hasLetterError(addOnData.addOn)}
          className='btn-primary'
        >
