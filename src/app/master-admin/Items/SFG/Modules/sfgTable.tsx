...
import {
  ...
  GetApp as GetAppIcon, Upload as UploadIcon,
  Description as DescriptionIcon,
  Undo as UndoIcon, Search as SearchIcon
} from '@mui/icons-material';
import { ExportCSV, ExportSfgheader, fetchSfg, ImportSfg, rollbackSfg, setSnackbarMessage, setSnackbarOpen } from '../Features/sfgSlice';
...
interface SfgTableContainerProps {
  handleEdit: (sfg: SFG) => void;
  handleDeactivate: (sfg: SFG) => void;
  handleActivate: (sfg: SFG) => void;
  handleOpen: () => void;
...
const SfgTableContainer: React.FC<SfgTableContainerProps> = ({
  handleEdit,
  handleDeactivate,
  handleActivate,
  handleOpen,
  ...
  const displayedSfg = showDeactivated ? deactivatedItems : sfg;
  ...
  const isAnyActionLoading = isImporting || isDownloadingSample;
  ...
  // // Handle import submission with selected mode
  ...
  //   try {
  //     await dispatch(ImportSfg({ file: selectedFile, mode: importMode })).unwrap();
  //     dispatch(fetchSfg({ search: '', page: 1 }));
  ...
  //         // Show general error in snackbar (already handled in slice, but as fallback)
  ...
  try {
    ...
    const result = await dispatch(
      ImportSfg({ file: selectedFile, mode: importMode })
    ).unwrap();
    ...
    if (error && typeof error === 'object') {
      ...
      dispatch(setSnackbarMessage('Failed to import SFG data'));
      dispatch(setSnackbarOpen(true));
    }
  } finally {
    setIsImporting(false);
  ...
  const handleRollback = async () => {
    ...
    setConfirmationAction(() => async () => {
      try {
        await dispatch(rollbackSfg()).unwrap();
        dispatch(fetchSfg({ search: '', page: 1 }));
  ...
  const handleDownloadSampleCSV = async () => {
    setIsImporting(true);
    try {
      await dispatch(ExportSfgheader()).unwrap();
  ...
  return (
    <>
      ...
      >
        ...
        >
          {showDeactivated ? "Deactivated SFG" : "Active SFG"}
        </Typography>
        ...
        <table className="custom-table">
          ...
          <tbody>
            {loading ? (
              ...
              <>
                {displayedSfg.map((sfg, index) => (
                  <tr key={sfg.id || index}>
                    <td style={{ textAlign: 'center' }}>{index + 1}</td>
                    <td style={{ textAlign: 'center' }}>{sfg.sfgCode}</td>
                    <td style={{ textAlign: 'center' }}>{sfg.sfgName}</td>
                    <td style={{ textAlign: 'center' }}>{sfg.price}</td>
                    <td style={{ textAlign: 'center' }}>{sfg.uom}</td>
                    <td style={{ textAlign: 'center' }}>{sfg.shelfLife}</td>
                    <td style={{ textAlign: 'center' }}>
                      <div className="flex justify-center gap-1">
                        {showDeactivated ? (
                          <button
                            onClick={() => handleActivate(sfg)}
                ...
                {displayedSfg.length === 0 && (
                  <tr>
                    <td colSpan={6} className="empty-state">
                      <h2>
                        {showDeactivated ? 'No deactivated SFG found' : 'No active SFG found'}
                      </h2>
                    </td>
                  </tr>
                )}
        ...
        <Box display="flex" justifyContent="center" mt={0.5}>
          <Pagination
            ...
            onChange={(event, value) => {
              dispatch(fetchSfg({ search: searchValue, page: value }));
            }}
          />
        </Box>
      )}
...
export default SfgTableContainer;