...
export default function PurchaseHome() {
  ...
  const purchaseModule = defaultModules.find((module) => module.id === 'purchase');
  const reports = purchaseModule?.children ?? [];
    ...
    <div className={`flex h-full min-h-0 w-full flex-col ${dark ? 'bg-slate-950' : 'bg-slate-50'}`}>
      <div className={`shrink-0 border-b px-4 py-3 ${dark ? 'border-slate-800 bg-slate-900' : 'border-slate-200 bg-white'}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className={`text-lg font-black ${dark ? 'text-white' : 'text-slate-950'}`}>Purchase Reports</h1>
            <p className={`text-xs ${dark ? 'text-slate-400' : 'text-slate-500'}`}>Choose a purchase report to continue</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {reports.map((report) => (
            ...
            >
              ...
              <HiOutlineArrowRight className="shrink-0 text-blue-600 transition group-hover:translate-x-0.5" size={16} />
            </Link>
          ))}
        </div>
      </div>
