...
interface PhotoDisplayProps {
  orderId: string;
  imageUrls?: string[];   // 0-based array from Redux
  canAdd: boolean;
  canEdit: boolean;
...
const PhotoDisplay: React.FC<PhotoDisplayProps> = ({
  ...
  const displayUrls = Array(3).fill('').map((_, i) => safeUrls[i] || '');

  const handleMouseEnter = (index: number, event: React.MouseEvent) => {
    if (!displayUrls[index]) return;
    const rect = event.currentTarget.getBoundingClientRect();
    setPreviewPosition({ top: rect.top - 210, left: rect.left - 50 });
              ...
              {url ? (
                ...
                <Tooltip title={canAdd ? 'Upload Photo' : 'No permission to upload'}>
                  <span style={{ display: 'inline-flex' }}>
                    <IconButton
                      onClick={() => canAdd && onUploadClick(orderId, displayIndex)}
                      disabled={!canAdd}
                      sx={{
